...
export default function ConvertPanel({ patient, sendConfig }: ConvertPanelProps) {
  ...
  const handleConvert = async (shouldSend: boolean) => {
    ...
    try {
      const result = await convertAndSend(file, patient, sendConfig, shouldSend);
      setStatus(result.success ? "success" : "error");
      setResultMsg(result.message);
      if (result.filename) setDicomFilename(result.filename);
    } catch {
      setStatus("error");
      setResultMsg("Erro ao converter. Verifique o backend.");
    }
  };
    ...
    <Card className="p-6 shadow-card">
      ...
      <div className="flex gap-3 mb-4">
        <Button
          ...
          disabled={!isReady || status === "converting"}
          variant="outline"
      ...
      {/* Download link */}
      {dicomFilename && status === "success" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-3">
          <a
            href={getDownloadUrl(dicomFilename)}
            download
